...
import { Plus, Search, Users, ArrowUpDown, ArrowUp, ArrowDown, MoreHorizontal, Pencil, Trash2, KeyRound, AtSign } from "lucide-react";
...
type SortField = "name" | "email" | "phone" | "city" | "createdAt";
    ...
    <form onSubmit={onSubmit} className="space-y-6 py-4">
      ...
      {showAccountFields && (
        <div className="space-y-4">
          ...
          <p className="text-sm text-slate-500">Create portal login credentials for this client. They will be able to log in and track their cases.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
...
export default function CROClients() {
  ...
  const { data: clients, isLoading } = trpc.cro.myClients.useQuery();

  const createMutation = trpc.cro.createClient.useMutation({
    onSuccess: () => {
      toast.success("Client created successfully");
      setIsCreateOpen(false);
      setFormData(emptyForm);
      utils.cro.myClients.invalidate();
      utils.cro.dashboardStats.invalidate();
    },
    onError: (err) => toast.error(err.message),
  });

  const updateMutation = trpc.cro.updateClient.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = trpc.cro.deleteClient.useMutation({
    onSuccess: () => {
  ...
  const resetPasswordMutation = trpc.cro.resetClientPassword.useMutation({
    onSuccess: () => {
  ...
  const updateUsernameMutation = trpc.cro.updateClientUsername.useMutation({
    onSuccess: () => {
      ...
      <div className="space-y-4 sm:space-y-6 md:space-y-8">
        <div className="border-b border-slate-200 pb-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-serif font-bold text-slate-900 tracking-tight">My Clients</h1>
              <p className="text-slate-600 mt-2 text-lg">Manage your client records</p>
            </div>
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
              ...
              <DialogContent className="max-w-[calc(100%-2rem)] sm:max-w-3xl max-h-[calc(100vh-2rem)] overflow-y-auto">
                <DialogHeader>
                  ...
                  <DialogDescription>Create a client record for credit repair services</DialogDescription>
                </DialogHeader>
                <ClientForm
                  formData={formData}
        ...
        <Card className="border-slate-200 shadow-sm">
          <CardContent className="pt-6">
            <div className="relative">
              ...
              <Input placeholder="Search clients by name or email..." value={searchQuery} onChange={(e) => handleSearchChange(e.target.value)} className="pl-10 border-slate-300" />
            </div>
          </CardContent>
        </Card>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50">
                    ...
                    <TableHead>
                      <button className="flex items-center font-medium hover:text-amber-600 transition-colors" onClick={() => handleSort("city")}>
                    ...
                    <TableHead className="w-[60px] text-center">Portal</TableHead>
                  ...
                  {paginatedClients.map((client) => (
                    <TableRow key={client.id} className="hover:bg-amber-50/50">
                      ...
                      <TableCell className="text-center">
                        {client.portalAccess ? (
                          <Badge variant="default" className="bg-green-100 text-green-700 border-green-200 text-xs">Yes</Badge>
                        ) : (
                          <Badge variant="outline" className="text-slate-400 text-xs">No</Badge>
                        )}
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
          ...
          <Card className="border-slate-200">
            <CardContent className="py-16 text-center">
              ...
              <p className="text-slate-600 text-lg mb-6">
                {searchQuery ? "No clients match your search criteria" : "Begin by adding your first client"}
              </p>
              {!searchQuery && (
